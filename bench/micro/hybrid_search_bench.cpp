...
// Use real IndexManager in benchmarks (no mocking).
} // namespace
...
static void BM_HybridSearch_Sequential(benchmark::State& state) {
    ...
    // Setup indices
    auto dense_index = std::make_shared<IndexManager>(dim);
    ...
    std::vector<float> all(n_docs * dim);
    for (std::size_t i = 0; i < n_docs; ++i) {
        std::copy(embeddings[i].begin(), embeddings[i].end(), all.begin() + i * dim);
    }
    IndexBuildConfig cfg; cfg.strategy = SelectionStrategy::Auto;
    auto b = dense_index->build(all.data(), n_docs, cfg);
    if (!b) { state.SkipWithError("IndexManager build failed"); return; }
