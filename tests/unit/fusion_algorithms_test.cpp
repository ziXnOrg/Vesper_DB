...
// Helper to check if results are sorted by score (FusedResult)
...
// Overload for SearchResult lists (sorted by raw score descending)
bool are_results_sorted(const std::vector<SearchResult>& results) {
    for (std::size_t i = 1; i < results.size(); ++i) {
...
TEST_CASE("Fusion utility functions", "[fusion]") {
    ...
    SECTION("Merge and deduplicate results") {
        ...
        REQUIRE(are_results_sorted(merged));
