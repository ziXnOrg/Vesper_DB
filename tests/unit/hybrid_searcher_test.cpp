...
class MockIndexManager : public IndexManager {
public:
    MockIndexManager(std::size_t dim) : dim_(dim) {
...
// Create test corpus
void setup_test_corpus(BM25Index& bm25, MockIndexManager& dense) {
...
TEST_CASE("HybridSearcher construction", "[hybrid]") {
    auto dense_index = std::make_shared<MockIndexManager>(128);
...
TEST_CASE("HybridSearcher query routing", "[hybrid]") {
    ...
    SECTION("DENSE_FIRST strategy") {
        ...
        for (const auto& result : *results) {
            REQUIRE(result.dense_score > 0.0f || result.sparse_score > 0.0f);
        }
    }
...
TEST_CASE("HybridSearcher with filters", "[hybrid]") {
    ...
    SECTION("Search with document filter") {
        ...
        for (const auto& result : *results) {
            REQUIRE(filter->contains(result.doc_id));
        }
    }
}
