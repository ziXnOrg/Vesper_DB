...
auto FastScanPq::encode(const float* data, std::size_t n,
    ...
    for (int i_i = 0; i_i < static_cast<int>(n); ++i_i) {
        ...
        std::uint8_t* code = codes + static_cast<std::size_t>(i_i) * config_.m;
...
auto FastScanPq::decode(const std::uint8_t* codes, std::size_t n, float* data) const -> void {
    ...
    #pragma omp parallel for
    for (int i_i = 0; i_i < static_cast<int>(n); ++i_i) {
        const std::uint8_t* code = codes + static_cast<std::size_t>(i_i) * config_.m;
        float* vec = data + static_cast<std::size_t>(i_i) * dim;
        for (std::uint32_t sub = 0; sub < config_.m; ++sub) {
            const std::uint32_t idx = sub * ksub_ + static_cast<std::uint32_t>(code[sub]);
            const float* centroid = (*codebooks_)[idx];
            std::copy(centroid, centroid + dsub_, vec + sub * dsub_);
        }
    }
}

auto FastScanPq::export_codebooks(std::vector<float>& out) const -> void {
    const std::size_t rows = static_cast<std::size_t>(config_.m) * static_cast<std::size_t>(ksub_);
    out.resize(rows * dsub_);
    for (std::uint32_t sub = 0; sub < config_.m; ++sub) {
        for (std::uint32_t code = 0; code < ksub_; ++code) {
            const std::uint32_t idx = sub * ksub_ + code;
            const float* src = (*codebooks_)[idx];
            std::copy(src, src + dsub_, out.data() + static_cast<std::size_t>(idx) * dsub_);
        }
    }
}
...
auto FastScanPq::import_pretrained(std::size_t dsub, std::span<const float> data) -> void {
    dsub_ = dsub;
    const std::size_t rows = static_cast<std::size_t>(config_.m) * static_cast<std::size_t>(ksub_);
    codebooks_ = std::make_unique<AlignedCentroidBuffer>(static_cast<std::uint32_t>(rows), dsub_);
    for (std::uint32_t sub = 0; sub < config_.m; ++sub) {
        for (std::uint32_t code = 0; code < ksub_; ++code) {
            const std::uint32_t idx = sub * ksub_ + code;
            const float* src = data.data() + static_cast<std::size_t>(idx) * dsub_;
            std::copy(src, src + dsub_, (*codebooks_)[idx]);
        }
    }
    trained_ = true;
}
