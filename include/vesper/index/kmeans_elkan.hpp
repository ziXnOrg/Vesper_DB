 ...
 *
 * Thread-safety: Internally parallelized but not thread-safe for concurrent calls.
 ...
 * - s[j]: Half the minimum distance from centroid j to any other centroid
 */
class KmeansElkan {
public:
...
private:
    ...
    static std::vector<std::vector<float>> inter_centroid_dist_cache_;
};
